...
fn gallery_entries() -> Vec<(String, String)> {
    ...
    for n in 1..=12 {
        entries.push((
            format!("Water {n:02}"),
            format!("{DIR}/shallow_loop_base_{n:02}.png"),
        ));
    }
    for (label, path) in [
        ("Edge N", EDGE_N),
...
mod tests {
    ...
    fn every_supported_combination_has_a_tile() {
        ...
        let with_tile = (0u8..16)
            .filter(|c| tile_path_for_code(*c).is_some())
            .count();
    ...
    fn dual_grid_samples_the_four_corners_at_a_coast() {
        ...
        assert_eq!(
            tile_path_for_code(dual_grid_code(&water, 0, 2)),
            Some(BASE_WATER)
        );
    }
    ...
    fn single_water_cell_makes_inner_corners() {
        ...
        assert_eq!(
            tile_path_for_code(dual_grid_code(&water, 5, 5)),
            Some(INNER_SE)
        );
        assert_eq!(
            tile_path_for_code(dual_grid_code(&water, 6, 5)),
            Some(INNER_SW)
        );
        assert_eq!(
            tile_path_for_code(dual_grid_code(&water, 5, 6)),
            Some(INNER_NE)
        );
        assert_eq!(
            tile_path_for_code(dual_grid_code(&water, 6, 6)),
            Some(INNER_NW)
        );
    }
